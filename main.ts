...
const atmWorking = async (users: User[]) => {
...
const atmFunc = async (user: User) => {
  ...
  if (ans.select === "Withdraw Money") {
    ...
    if (amount.money > 25000) {
      return console.log(
        "Sorry! You are not allowed to withdraw more than 25000 in one transaction"
      );
    }
  ...
  if (ans.select === "Check Balance") {
    return console.log(
      ` Thankyou ${user.name} for using this ATM. Your balance is = ${user.balance}`
    );
  }
